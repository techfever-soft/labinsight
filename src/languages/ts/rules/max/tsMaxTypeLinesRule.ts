...
export class TSMaxTypeLinesRule implements LabInsightRule {
  ...
  public async apply(
    ...
    const result = this.analyzeTypeLines(sourceFile, filePath);

    return result;
  }

  private analyzeTypeLines(
    ...
    const visit = (node: ts.Node) => {
      if (ts.isTypeAliasDeclaration(node)) {
        ...
        const typeLines = endLine - startLine + 1;

        if (typeLines > this.limit) {
          response = {
            path: filePath,
            message: `The type has ${typeLines} lines, which is more than the allowed limit of ${this.limit} lines`,
            severity: "error",
          };
        }
      }
