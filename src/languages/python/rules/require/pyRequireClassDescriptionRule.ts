import {
  ...
  export class PyRequireClassDescriptionRule implements LabInsightRule {
    ...
    public async apply(
      ...
      return this.checkClassDescriptions(ast, filePath);
    }
  
    /**
     * Check if classes in the AST have a description (docstring)
     * @param ast The AST of the Python file
     * @param filePath The path of the analyzed file
     * @returns LabInsightRuleResponse[]
     */
    private checkClassDescriptions(ast: any, filePath: string): LabInsightRuleResponse[] {
      const responses: LabInsightRuleResponse[] = [];
  
      // Helper function to check each class
